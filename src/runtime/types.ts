...
import { useQueryClient, useQuery, useMutation, keepPreviousData, useInfiniteQuery } from '@tanstack/vue-query'
...
export class ApiError {
  ...
  content?: object
  cause?: object
  constructor(statusCode: number, status: string, message: string, content?: object, cause?: object) {
...
export class User {
  ...
  constructor(username: string, password: string, fullName: string, email: string, token: string) {
    this.username = username
    this.password = password
...
export class Constraint {
  ...
  constructor(matchMode: string, value: unknown) {
...
export class Sort {
  ...
  constructor(property: string, direction: 'asc' | 'desc' = 'asc') {
...
export class Page<T> {
  ...
  content: T[]
  constructor(page: { number: number, size: number, totalElements: number, totalPages: number }, content: T[]) {
    this.page = page
    this.content = content
  }
}
...
//   public login: UseQueryReturnType<User, ApiError>
//   protected service: LoginService
//   constructor(baseUrl: string, login: Login) {
//     this.service = new LoginService(baseUrl)
//     this.login = useQuery({
//       queryKey: ['login', login],
//       queryFn: async () => await this.service.login(login),
//     })
//   }
// }
